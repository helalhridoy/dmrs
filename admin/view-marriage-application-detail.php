...
{   
   ...
   $nid_number_hus=htmlentities($row->HusbandNidno);
    $nid_name_hus=htmlentities($row->HusbandName);
    $nid_dob_hus=htmlentities($row->Husbanddob);
    $nid_number_wife=htmlentities($row->WifeNidNo);
    $nid_name_wife=htmlentities($row->WifeName);
  ?>
...
Photo of Groom
   ...
   <tr>
    ...
    <th scope>Nid Number</th>
    <td><?php  echo $row->HusbandNidno;?></td>
  </tr>
  <tr>
  <td colspan="8" style="font-size:20px;color:red">
 Wife Details</td></tr>
 ...
 <tr>
  ...
  <tr>
    <img src="../user/images/<?php echo $row->nikahimg;?>" width="600" >
  </tr>
  <tr>
...
<?php } else { ?>                  <td colspan="4"><?php  echo htmlentities($row->Status);?>
  ...
  <tr>
    <th>Husband Info Check</th>
    <td rowspan="3">
                 
                  <?php
                       
                       if(isset($_POST['btnoik2']))
                       {
                        $rdb = new firebaseRDB($databaseURL);
                        $retriev = $rdb->retrieve("/nid","nid_number","EQUAL",$nid_number_hus);
                        $datas = json_decode($retriev,1);
                        if(is_array($datas))
                        {
                            foreach($datas as $data)
                          {
                            ...
                            {
                              echo "<strong>Husband data not found on NID API</strong>";
                            } else
                            {
                                if($nid_name_hus==$data["nid_name"])
                                {
                                  echo("<strong>Data Verified</strong>");
                                }
                                else
                                {
                                  echo("<strong>Name didn't matched</strong>");
                                }
                            }
                          }
                        }
                        ...
                        {
                          echo "<strong>Husband data not found</strong>";
                        }
    ...
    <form action="" method="post">
                <input type="submit" value="Verify Data with National ID Demo Server" name="btnoik2">
    ...
    <td>
                    <?php
                     ...
                     {
                        $rdb = new firebaseRDB($databaseURL);
                        $retriev2 = $rdb->retrieve("/nid","nid_number","EQUAL",$nid_number_wife);
                        $datas2 = json_decode($retriev2,1);
                        
                         foreach($datas2 as $data2)
                          {
                           
                            if(!isset($data2['nid_number']))
                            {
                              echo " <strong>fe data not found on NID API</strong>";
                            } else
                            {
                                if($nid_name_wife==$data2["nid_name"])
                                {
                                  echo("<strong>Data Verified</strong>");
                                }
                                else
